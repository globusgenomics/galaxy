...
class FastQCRunner(object):
    ...
    def prepare_command_line(self):
	   ...
	   try:
        ...
        if 'fastq' in opts.informat:
            ...
            # accepted formats..
            opts.informat = 'fastq'
        elif 'gz' in opts.informat:
            self.fastqinfilename = self.fastqinfilename + ".fastq.gz"
        elif not self.fastqinfilename.endswith(opts.informat):
            self.fastqinfilename += '.%s' % opts.informat

        if self.file_type(self.opts.input) != "txt" and not self.fastqinfilename.endswith(".gz"):
            self.fastqinfilename = self.fastqinfilename + ".fastq.gz"
    ...
    def run_fastqc(self):
        ...
        sout.write('\n')
        if not os.path.islink(self.fastqinfilename):
