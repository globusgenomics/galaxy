...
import sys, optparse, os, tempfile, subprocess, shutil, vcf, pysam, re
...
##liubo added
    ...
    config_inputs = "%s/%s" % (tmp_dir, "inputs.txt.list")
    ...
    bam_filenames = []
    fh_inputsamples = open(config_inputs, "w")
    fh_inputsamples.write("Sample ID\tBam File\tNotes\n")
    if options.datasets:
        for ( dataset_arg, filename, galaxy_ext, prefix ) in options.datasets:
            ...
            if galaxy_ext == "bam":
                ...
                sn = get_bam_samplename(gatk_filename)
                notes = "notes"
                fh_inputsamples.write("%s\t%s\t%s\n" % (sn, gatk_filename, notes))
                #print "%s\t%s\t%s" % (sn, gatk_filename, notes)

    fh_inputsamples.close()
    cmd = "%s -s %s" % (cmd, config_inputs)
