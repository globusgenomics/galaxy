...
def __main__():
    ...
    for seq in forward:
        ...
        # submit the sample
        #cmd = "PACKAGE_BASE=/mnt/galaxyTools/tools/samtools/0.1.19; export PACKAGE_BASE; . /mnt/galaxyTools/tools/samtools/0.1.19/env.sh; PACKAGE_BASE=/mnt/galaxyTools/tools/bwa/0.7.12; export PACKAGE_BASE; . /mnt/galaxyTools/tools/bwa/0.7.12/env.sh; PACKAGE_BASE=/mnt/galaxyTools/tools/pymodules/python2.7; export PACKAGE_BASE; . /mnt/galaxyTools/tools/pymodules/python2.7/env.sh; PACKAGE_BASE=/mnt/galaxyTools/tools/R/3.2.2; export PACKAGE_BASE; . /mnt/galaxyTools/tools/R/3.2.2/env.sh; python %s/CGI_m3p_tumor_optimized_05012016.py --sample-name %s --bwa-ref %s --fastq %s --rfastq %s --upper %s --lib-size %s --read-length %s --output-dir %s --output-log %s --min-pairs %s --rev-comp %s --input-normal %s" % (options.script_source, sample_name, options.bwa_ref, "%s/%s" % (options.input_dir, fastq), "%s/%s" % (options.input_dir, rfastq), options.upper, options.lib_size, options.read_length, output_dir, output_log, options.min_pairs, options.rev_comp, options.input_normal)
        cmd = "python %s/CGI_m3p_tumor_optimized_05012016.py --source %s/called --sample-name %s --bwa-ref %s --fastq %s --rfastq %s --upper %s --lib-size %s --read-length %s --output-dir %s --output-log %s --min-pairs %s --rev-comp %s --input-normal %s" % (options.script_source, options.script_source, sample_name, options.bwa_ref, "%s/%s" % (options.input_dir, fastq), "%s/%s" % (options.input_dir, rfastq), options.upper, options.lib_size, options.read_length, output_dir, output_log, options.min_pairs, options.rev_comp, options.input_normal)
    ...
    cmd = "python %s/CGI_m3p_merge_optimized_05012016.py --source %s/called --sample-name %s --lib-size %s --output-dir %s --output-log %s %s" % (options.script_source, options.script_source, options.project_name, options.lib_size, project_dir, output_log, samples)
