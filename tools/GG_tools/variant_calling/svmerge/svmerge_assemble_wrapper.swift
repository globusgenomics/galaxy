...
file inputfiles[] <ext; exec="/nfs/software/galaxy/tools/GG_tools/variant_calling/svmerge/file_mapper.sh", location=filedir>;
...
foreach f,i in inputfiles {
  ...
  string cmd = @regexp(@arg("app_cmd"), "(--svconfig)", @strcat("/",@filename(f)));
  trace(cmd); 
