...
def gatk_filename_from_galaxy( galaxy_filename, galaxy_ext, target_dir = None, prefix = None, picard_jar = None, reformat_vcf = None):
    ...
    if reformat_vcf is None:
        ...
        if suffix == "vcf" and os.path.exists("%s.idx" % galaxy_filename) and not os.path.basename(galaxy_filename).endswith("gz"):
            os.symlink( "%s.idx" % galaxy_filename, "%s.idx" % gatk_filename)
        elif suffix == "vcf" and os.path.basename(galaxy_filename).endswith("gz"):
            #print "IAM HERE: %s, %s.gz" % (galaxy_filename,  gatk_filename)
            os.symlink( "%s" % galaxy_filename, "%s.gz" % gatk_filename)
            gatk_filename = "%s.gz" % gatk_filename
    else:
...
def __main__():
    ...
    parser.add_option( '-t', '--tool', dest="tool_name", action="store", type="string", help="GATK tool name to be run" )
    ...
    if options.max_jvm_heap is not None:
        cmd += "--java-options -Xmx%s " % ( options.max_jvm_heap )
    elif options.max_jvm_heap_fraction is not None:
        cmd += "--java-options -XX:MaxRAMFraction=%s " % (options.max_jvm_heap_fraction)
    cmd += "%s " % options.tool_name
    ...
    if options.datasets:
        ...
        for ( dataset_arg, filename, galaxy_ext, prefix ) in options.datasets:
            ...
            if dataset_arg:
            #if dataset_arg and options.tool_name != "HaplotypeCaller":
                cmd = '%s %s "%s"' % ( cmd, gatk_filetype_argument_substitution( dataset_arg, galaxy_ext ), gatk_filename )
            #elif dataset_arg and options.tool_name == "HaplotypeCaller":
            #    print "FILENAME: %s" % filename
            #    variant_files.append(gatk_filename)

            if galaxy_ext == "bam":
                bam_filenames.append( gatk_filename )
            elif galaxy_ext == "vcf" or galaxy_ext == "bed":
                if not os.path.exists("%s.idx" % gatk_filename) and not os.path.basename(filename).endswith("gz"):
                    index_cmds.append("gatk IndexFeatureFile -F %s" % gatk_filename)
                elif galaxy_ext == "vcf" and os.path.basename(filename).endswith("gz"):
                    index_cmds.append("gatk IndexFeatureFile -F %s" % gatk_filename)
    ...
    if interval_count == 0 and options.tool_name == "GenomicsDBImport":
        cmd = "%s -L 20" % cmd
    elif len(variant_files) > 0 and options.tool_name == "HaplotypeCaller":
        variant_list_name = "%s/var_list.txt" % (tmp_dir)
        fh_var_list = open(variant_list_name, "w")
        fh_var_list.write("\n".join(variant_files))
        fh_var_list.close()

        cmd = "%s -V %s" % (cmd, variant_list_name)
 
    if len(index_cmds) > 0: 
        cmd = "%s; %s" % (";".join(index_cmds), cmd)
    print cmd
