...
parser.add_argument ( '-t', dest='types', help='SV analysis type (DEL, DUP, INV, TRA), INS', nargs='+', required=True )
...
parser.add_argument ( '-f', dest='flanking', help='quality of the consensus alignment', default=90 )
parser.add_argument ( '-m', dest='min_flank', help='minimum flanking sequence size', default=13 )
parser.add_argument ( '-n', dest='noindels', help='no small InDel calling?', action='store_true')
parser.add_argument ( '-i', dest='indelsize', help='max. small InDel size', default=500)
parser.add_argument ( '-v', dest='vcffile', help='input VCF/BCF file for re-genotyping')
parser.add_argument ( '-u', dest='geno_qual', help='min. mapping quality for genotyping', default='5' )
...
def delly(args, tempDir):
	...
	for typ in args.types:
		...
		cmd = "%s call %s -t %s -o %s -q %s -s %s -f %s -m %s -i %s -u %s " % (dellyPath, args.bam, typ, output, args.map_qual, args.mad_cutoff, args.flanking, args.min_flank, args.indelsize, args.geno_qual)
		if (args.genome!="" and args.genome!=None):
			cmd += " -g %s" % (args.genome)
                if (args.exclude != "" and args.exclude!=None):
                        cmd += " -x %s" % (args.exclude)
                if (str(args.noindels) != "" and args.noindels!=None):
                        cmd += " %s" % (args.noindels)
                if (args.vcffile != "" and args.vcffile!=None):
                        cmd += " -v %s" % (args.vcffile)                
		print ("command = "+cmd)
		execute( cmd )
	return tempOutputs
