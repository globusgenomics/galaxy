...
template <class ObjectT> class LongHash
   ...
   public:
      ...
      long long GetKey(const int i)             { return keys[i]; }
      long long GetKey(const unsigned int i)    { return keys[i]; }
      ...
      bool SlotInUse(int index) { return occupancy[index] == true; }
      bool SlotInUse(unsigned int index) { return occupancy[index] == true; }
