...
file mzmlfiles[] <ext; exec="/opt/galaxy/tools/protk/protk-255b5b6ec617/file_mapper.sh", location=mzmldir>;
...
foreach f,i in mzmlfiles {
  ...
  string mzml_outfile = @strcat(outputdir, "/", @regexp(input_mzml_basename, ".mzML", ""), ".pepXML");
  ...
  string output_flag = @strcat(" -o ",  mzml_outfile);
  string cmd2 = @regexp(cmd1, "(OUTPUT)", output_flag);

  trace(cmd2); 

  ## send the constructed command through the shell
  (outlog, errlog) = tandem_search(cmd2);
}
