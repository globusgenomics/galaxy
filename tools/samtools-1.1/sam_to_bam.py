...
def __main__():
    ...
    parser.add_option( '-p', '--pass_through', dest='pass_through_options', action='append', type="string", help='These options are passed through directly.' )
    ...
    try:
        ...
        pass_through = ""
        if options.pass_through_options is not None:
            pass_through = ' '.join( options.pass_through_options )
        command = 'samtools view -bt %s %s -o %s %s' % ( fai_index_file_path, ' '.join( options.pass_through_options ), tmp_aligns_file_name, options.input1 )
        print command
        tmp = tempfile.NamedTemporaryFile( dir=tmp_dir ).name
        tmp_stderr = open( tmp, 'wb' )
        proc = subprocess.Popen( args=command, shell=True, cwd=tmp_dir, stderr=tmp_stderr.fileno() )
