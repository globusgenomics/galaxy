...
def reformat_to_gz( fastq ):
    # reformat fastq file if it's in bz2 format to gz format
    # create tmp
    tmp = tempfile.NamedTemporaryFile(suffix=".gz").name      

    cmd = "bunzip2 -c < %s | gzip -c > %s" % (fastq, tmp)

    tmp_stdout = open( tmp, 'wb' )
    proc = subprocess.Popen( args=cmd, shell=True, stdout=tmp_stdout )
    tmp_stdout.close()
...
def __main__():
    ...
    parser.add_option( '', '--output-format', dest="output_format", help="The format of the output (SAM or BAM)" )
    ...
    if options.rfastq:
         ...
         if rfastq.endswith(".bz2"):
             # create temp and reformat to gz
             rfastq = reformat_to_gz(rfastq)
    ...
    if options.genAlignType == 'paired':
        ...
        if options.output_format == "BAM":
            cmd3 = 'bwa sampe %s %s %s %s %s %s | samtools view -bhS -o %s -' % ( gen_alignment_cmds, ref_file_name, tmp_align_out_name, tmp_align_out2_name, fastq, rfastq, options.output )
        else:
        ...
        if options.output_format == "BAM":
            cmd3 = 'bwa samse %s %s %s %s | samtools view -bhS -o %s -' % ( gen_alignment_cmds, ref_file_name, tmp_align_out_name, fastq, options.output )
        else:
