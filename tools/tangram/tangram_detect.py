...
def __main__():
    ...
    if options.dataset:
        ...
        cmd = "%s -in %s -lb %s -ht %s -ref %s -rg RG_NAME -p 1 " % (tangram_detect_path, "%s/in.txt" % wd_tmpdir, options.libFile, options.histFile, ref_index )
    else:
    ...
    if options.dataset:
        ...
        swift_params = list()
        swift_params.append('-input_bam=' + options.bamFile)
