...
file inputchrs[] <ext; exec="/nfs/software/galaxy/tools/tangram/tangram_detect_file_mapper.sh", bam=filedir>;
...
foreach f,i in inputchrs {
  ...
  string cmd1 = @regexp(@arg("app_cmd"), "(RG_NAME)", @strcat(@filename(f)));
  trace(cmd1); 

  string cmd = @regexp(cmd1, "(OUTVCF)", @strcat(outputdir, "/", @filename(f), ".vcf"));
  trace(cmd);
