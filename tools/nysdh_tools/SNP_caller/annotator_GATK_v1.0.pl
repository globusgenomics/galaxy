...
my $loci = "/opt/galaxy/tools/nysdh_tools/SNP_caller/Resistance_loci_v2.txt";  ## resistance loci list	
my $hq = "/opt/galaxy/tools/nysdh_tools/SNP_caller/High_confidence_list_v2_2.txt"; ## High confidence mutation list 
my $neutral_mutations_list = "/opt/galaxy/tools/nysdh_tools/SNP_caller/Neutral.lst";
			...
			if ($length_variant_call > 1) { ## mean insertion in variant
				...
				if (($size_insertion % 3)) {		## check for frameshift mutation using modulo			
					...
					if ($split_loci_line[4] =~ 'rpoB') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$pos\t+$join\tFrameshift INSERTION\t$heterogenecity{$start}";
					...
					my $frag_start=0;
					do {
						my $codon = $tmp[$frag_start].$tmp[$frag_start+1].$tmp[$frag_start+2];						
					...
					if ($split_loci_line[4] =~ 'rpoB') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$pos\t+$join\t+$frag\tIn-Frame INSERTION\t$heterogenecity{$start}";	
						$hash_loci{$split_loci_line[4]} .= "separatorINSERTION";								
						$antibio{$split_loci_line[5]} = 1;
					}
				}	
				...
				if (($size % 3)) {						
					...
					if ($split_loci_line[4] =~ 'rpoB') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$pos\t-$join\tFrameshift DELETION\t$heterogenecity{$start}";
					...
					if ($split_loci_line[4] =~ 'rpoB') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$pos\t-$join\t-$frag\tIn-Frame DELETION\t$heterogenecity{$start}";
						$hash_loci{$split_loci_line[4]} .= "separatorDELETION";
						$antibio{$split_loci_line[5]} = 1;	
					}
				}
...
### look for Codon based mutations
	...
	### look for mutation on positiv strand in intergenic positions ######
		...
		do {	
			...
			if (($position{$start} !~ $wt{$start}) && ($position{$start} !~ 'N')) {			
				if ($length_variant_call > 1) {			## look for Indels ##				
					...
					shift @tmp;
					my $join = join ('',@tmp);
					$string .= "$real_loci_start\t$loc\tItergenic Insertion\t+$join\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n";
					...
					$string .= "$real_loci_start\t$loc\tItergenic deletion\t$position{$start}\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n";
					push (@mutations,"$real_loci_start\t$loc\tItergenic DELETION\t$position{$start}\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n");
					$antibio{$split_loci_line[5]} = 1;
				} else {	
	...
	### look for mutation on positiv strand in ribosomal RNA positions ######
			...
			if ($length_variant_call > 1) {
				...
				if (($size % 3)) {					
					...
					if ($split_loci_line[4] !~ 'gidB|iniB|embR') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$loc\t+$join\tFrameshift INSERTION\t$heterogenecity{$stop_position_5prime}";
						$hash_loci{$split_loci_line[4]} .= "separatorINSERTION";
						$antibio{$split_loci_line[5]} = 1;
					}
				} else {								
					...
					if ($split_loci_line[4] =~ 'katG|pncA') {
						push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$loc\t+$join\t+$frag\tIn-Frame INSERTION\t$heterogenecity{$stop_position_5prime}";
						$hash_loci{$split_loci_line[4]} .= "separatorINSERTION";
						$antibio{$split_loci_line[5]} = 1;
					}
				...
				if ($size > 0) {
					if (($size % 3)) {
						...
						if ($split_loci_line[4] =~ 'katG|pncA|ethA') {
							push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$loc\t-$join\tFrameshift DELETION\t$heterogenecity{$stop_position_5prime}";
							$hash_loci{$split_loci_line[4]} .= "separatorDELETION";	
							$antibio{$split_loci_line[5]} = 1;		
						}			
					} else {
						...
						if ($split_loci_line[4] =~ 'katG|pncA') {
							...
							push @frameshift,"$split_loci_line[5]\t$split_loci_line[4]\t$loc\t-$join\t$frag\tIn-Frame DELETION\t$heterogenecity{$stop_position_5prime}";	
							$hash_loci{$split_loci_line[4]} .= "separatorDELETION";	
						}
					}
				}		
    ...
    ### annotate snp in intergenic region on negative strand ###
		...
		do {
			...
			if ($length_variant_call>1) {			## look for Indels ##
				...
				my $size = @tmp;
				$start_position_3prime= $start + $size;
				$string .= "$start_position_3prime\t$loc\tItergenic Insertion\t+$join\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n";
				$antibio{$split_loci_line[5]} = 1;
				...
				my $size = @tmp;
				$start_position_3prime= $start + $size;			
				$string .= "$start_position_3prime\t$loc\tItergenic deletion\t-$join\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n";	
				$antibio{$split_loci_line[5]} = 1;
			} else {			
				$string .= "$start\t$loc\t$wt{$start} -> $position{$start}\t$split_loci_line[3]\t$split_loci_line[4]\t$split_loci_line[5]\t$heterogenecity{$start}\n";
				$antibio{$split_loci_line[5]} = 1;
...
while (my $length_WT_call=<TMP>) {
	...
	$neutral_pos{$split_line [0].$split_line [1].$split_line [2]} = 1;
}
close TMP;

my @list = split ('\n',$string);
foreach $length_WT_call(@list) {
	if ($length_WT_call !~ 'rrs1|rrs2') {
		@split_line = split ('\t',$length_WT_call);

		if (exists $neutral_pos{$split_line[5].$split_line[1].$split_line[3]}) {
			print "$length_WT_call (Neutral)\n";
		} else {
			print "$length_WT_call\n";
		}
	}
}
		...
		#### look for any non-silent mutation in certain loci flagged 'Any'
		 	...
		 	foreach my $key (keys %hash_aa) {		 
			 	if ($key =~ $split_HC_mutation_line[1]) {
					...
					elsif (($amino_acid_wt !~ $amino_acid_var) && ($amino_acid_var !~ 'X')) {
							@HC_aa = split ($split_HC_mutation_line[1],$key);
							$hash_loci{$split_HC_mutation_line[1]} .= "separator$amino_acid_wt$HC_aa[1]$amino_acid_var";
							$status="Resistant";
							$pass=0;
			 ...
			 foreach my $key (keys %hash_nt) {
				if ($key =~ $split_HC_mutation_line[1]) {
					if (($hash_nt_wt{$key} =~ $hash_nt{$key}) && ($status=~'ND'))  {
						$status="Susceptible";
					} elsif (($hash_nt_wt{$key} !~ $hash_nt{$key}) && ($hash_nt{$key} !~ 'N')) {
						@HC_aa = split ($split_HC_mutation_line[1],$key);
...
my @loci_list=('rpoB','katG','oxyR-ahpC promoter region','inhA','mabA-inhA promoter region','mabA','pncA','pncA promoter region','embB','rpsL','rrs1','rrs2','eis promoter region','gyrA','gyrB','ethA');
...
foreach my $next_loci(@loci_list) {
	...
	if (exists $hash_failed{$next_loci}) {
		if ($next_loci =~ 'katG') {  ## special case loop for katG to only call FAIL resistance if a high confidence site 
			if ($hash_failed{$next_loci} =~ 'or279se|or315se|or525se|or394se') {
		...
		if ($next_loci =~ 'promoter|rrs1|$rss2') {
			print "position ";
		} else {
			print "codon ";
		}
...
foreach $screen(@antimicrobial) {
	if ($resis{$screen}) {
		...
		$resis{$screen} =~ s/Susceptible/Susceptible (suggested)/g;
		$resis{$screen} =~ s/\t+/\t/;
