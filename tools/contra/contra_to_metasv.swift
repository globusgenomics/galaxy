...
foreach f,ix in itemNames {
    ...
    tracef("\n final_vcf=%s\n",final_vcf);
    file outlog <single_file_mapper; file=@strcat("logs/", tmp[0], ".contra_to_metasv.stdout.log")>;
    file errlog <single_file_mapper; file=@strcat("logs/", tmp[0], ".contra_to_metasv.stderr.log")>;
