...
def __main__():
    ...
    try:
        ...
        if options.order == "lexicographically":
            order_param = "-n"
        # create a symlink of the bam file with bam extension
        tmp_link = "%s/input.bam" % tempfile.mkdtemp()
        os.symlink(options.input, tmp_link)
        command = 'samtools rocksort -@ 28 %s -f %s %s' % ( order_param, tmp_link, options.output )
        print command
        tmp_dir = tempfile.mkdtemp()
...
#    subprocess.call(command,shell=True)
        ...
        #shutil.copyfile(generate_bam,options.output)
    except:
