...
SKIP_TOOL_IDS = ['get_bdbag_from_minid', 'create_output_bdbag_and_minid2', 'create_output_bdbag_and_minid', 'cwl_prepare_inputs']
...
key = options.api_key
#key = "hmpcvthfopjdthmpcvthfopjdt"
if len(key) == 0:
...
#cmd_globus = 'curl -X GET  https://auth.globus.org/v2/oauth2/userinfo  -H \'authorization: Bearer %s' %(globus_auth_token)
##print cmd_globus
...
##minid --register_user --email <email> --name <name> --globus_auth_token <token>
#cmd = "minid --register_user --email \"%s\" --name \"%s\" --globus_auth_token \"%s\"; minid --register --config %s --location %s --globus_auth_token \"%s\" --name \"%s\" --email \"%s\" --title \"%s\" %s" % (email, name, options.goauth, config_path, s3_url, options.goauth, name, email, minid_title, archived_path)
##print cmd
...
#print "MINID\t%s" % ( minid)
#print "MANIFEST\t%s" % (json.dumps(remote_manifest))
