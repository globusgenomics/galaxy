...
from_path = transfer_info['from_path'].strip().rstrip('/').rstrip('\\')
to_path = transfer_info['to_path'].strip().strip('/').strip('\\')
if transfer_info['sse'] in [True, False]:
    server_side_encryption = transfer_info['sse']
else:
    sys.exit("Failed to read SSE value.")

if transfer_info['rename'] != '':
    rename = transfer_info['rename'].strip().strip('/').strip('\\')
else:
    rename = None
