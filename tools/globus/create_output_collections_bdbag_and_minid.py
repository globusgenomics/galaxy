...
def get_filepath(UUID):
    GALAXY_DATABASE_CONN = "postgresql://galaxy:globus_genomics_pass@rds.ops.globusgenomics.org:5432/galaxy_nihcommonstest"
...
parser.add_option( '-t', '--token', dest="goauth", help='Globus auth token' )
(options, args) = parser.parse_args()


# skip these tool ids for bdbad 
SKIP_TOOL_IDS = ['get_bdbag_from_minid', 'create_output_bdbag_and_minid', 'create_output_bdbag_and_minid_from_collections']
...
for item in dataset_to_transfer:
    ...
    url_path = "globus://%s/%s/%s" % (local_ep, local_root_path, to_loc)
...
#cmd = "minid --register --config %s --location %s --globus_auth_token \"%s\" --name \"%s\" --email \"%s\" --title \"TOPMED RNA-seq output results for %s\" %s" % (config_path, s3_url, options.goauth, name, email, options.history_id, archived_path)
