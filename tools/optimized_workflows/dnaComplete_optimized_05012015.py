...
def create_wf_meta(options, output_dir, tmp_dir):
    ...
    cmd = "python /opt/galaxy/tools/gatk3/gatk3_wrapper.py --max_jvm_heap_fraction 1 --stdout %s -d \"-I\" \"%s\" \"bam\" \"gatk_input\" -d \"\" \"%s\" \"bam_index\" \"gatk_input\" -p \"java -jar %s/GenomeAnalysisTK.jar -T RealignerTargetCreator -o %s --num_threads %s -R %s\" -d \"-known:dbsnp,%%(file_type)s\" \"%s\" \"vcf\" \"input_dbsnp_0\" -d \"-known:indels,%%(file_type)s\" \"%s\" \"vcf\" \"input_indels_0\"" % (outputLog, inputF, inputIndex, GATK3_PATH, outputF, ncpu, reference_db, dbsnp, mills)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    output_files = [outputF, outputLog]
    cmd = "python /opt/galaxy/tools/gatk3/gatk3_wrapper.py --max_jvm_heap_fraction 1 --stdout %s -d \"-I\" \"%s\" \"bam\" \"gatk_input\" -d \"\" \"%s\" \"bam_index\" \"gatk_input\" -p \"java -jar %s/GenomeAnalysisTK.jar -T PrintReads -o %s -nct %s -R %s --BQSR %s\"" % (outputLog, inputBam, inputIndex, GATK3_PATH, outputF, ncpu, reference_db, inputTable)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    cmd = "python /opt/galaxy/tools/gatk3/gatk3_wrapper.py --max_jvm_heap %sm --stdout %s -d \"-I\" \"%s\" \"bam\" \"gatk_input_0\"  -d \"\" \"%s\" \"bam_index\" \"gatk_input_0\" -d \"--dbsnp:dbsnp,%%(file_type)s\" \"%s\" \"vcf\" \"input_dbsnp_0\" -p \"java -jar %s/GenomeAnalysisTK.jar -T HaplotypeCaller -nct %s --out %s -R %s --output_mode EMIT_VARIANTS_ONLY\"" % (jvm_heap, outputLog, inputBam, inputIndex, dbsnp, GATK3_PATH, ncpu, outputF, reference_db)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    input_files = [inputF]
    output_files = [outputF, outputLog]
    cmd = "python /opt/galaxy/tools/gatk3/gatk3_wrapper.py --max_jvm_heap %sm --stdout %s -d \"--variant:variant,%%(file_type)s\" \"%s\" \"vcf\" \"input_variant\" -p \"java -jar %s/GenomeAnalysisTK.jar -T VariantFiltration -o %s -R %s --maskExtension 0 --maskName Mask\" -d \"--mask:Mask,%%(file_type)s\" \"%s\" \"vcf\" \"input_mask_Mask\"" % (jvm_heap, outputLog, inputF, GATK3_PATH, outputF, reference_db, mask)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    input_files = [inputF]
    output_files = [outputF, outputLog]
    cmd = "python /opt/galaxy/tools/gatk3/gatk3_wrapper.py --max_jvm_heap %sm --stdout %s -d \"--variant:variant,%%(file_type)s\" \"%s\" \"vcf\" \"input_variant\" -p \"java -jar %s/GenomeAnalysisTK.jar -T SelectVariants --num_threads %s -o %s -R %s\"" % (jvm_heap, outputLog, inputF, GATK3_PATH, ncpu, outputF, reference_db)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
