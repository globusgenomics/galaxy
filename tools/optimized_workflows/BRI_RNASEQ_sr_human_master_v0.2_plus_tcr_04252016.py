...
"""
Runs the BRI RNA-seq sr_human_master_v0.2_plus_tcr pipeline in optimized mode in one Node
...
def create_wf_meta(options, output_dir, tmp_dir):
    ...
    outputLogRest = options.output_trinity_log
    output_files = [outputF, outputLog]
    cmd = "python /opt/galaxy/tools/trinity/trinityToolWrapper.py Trinity --output %s/trinity --max_memory %sG --CPU %s --single %s --seqType fq --min_kmer_cov 3 --max_reads_per_graph 20000000 --bflyHeapSpaceMax %sG > %s; cp %s %s; cp %s %s" % (output_dir, int(jvm_heap/2000), ncpu_step, inputF, int(jvm_heap/2000), outputLog, outputF, outputFrest, outputLog, outputLogRest )
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files": output_files})
    ...
    input_files = [input1]
    outputF1 = "%s/%s-mixcr.report.log" % (output_dir, sample_name)
    outputF2 = "%s/%s-mixcr.clones.txt" % (output_dir, sample_name)
    outputF3 = "%s/%s-mixcr.alignments.txt" % (output_dir, sample_name)
    outputFrest1 = options.output_mixcr_reports
    outputFrest2 = options.output_mixcr_clones
    outputFrest3 = options.output_mixcr_alignments
    output_files = [outputF1, outputF2, outputF3]
    cmd = "python /opt/galaxy/tools/mixcr/mixcr.py --input standard -p '--chains TCR' -p '--species %s' --report mixcrReport.log --parameters default --OassemblingFeatures VDJRegion --output %s --output-dir %s --output1 %s --output2 %s --inputfastq %s; cp %s %s; cp %s %s; cp %s %s" % (options.mixcr_ref, outputF1, output_dir, outputF2, outputF3, input1, outputF1, outputFrest1, outputF2, outputFrest2, outputF3, outputFrest3)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF1, "output_files": output_files})
    ...
    inputFindex = command_meta[step-1][2]["output_file"]
    inputList = options.annotation_snp_bed
    outputF1 = "%s/%s-mpileup1.stdout" % (output_dir, sample_name)
    outputF = "%s/%s-mpileup1.bcf" % (output_dir, sample_name)
    samtools_reference = options.samtools_reference
    input_files = [inputF]
    output_files = [outputF, outputF1]
    cmd = "python /opt/galaxy/tools/samtools/samtools_wrapper.py -p '%s/samtools mpileup' --stdout %s -p '-f %s' -d \" \" \"%s\" \"bam\" \"bam_input_0\" -d \"\" \"%s\" \"bam_index\" \"bam_input_0\" -p ' -C 0 -d 250 -l %s -q 0 -Q 13 -g -e 20 -h 100 -L 250 -o 40 > %s '" % (samtools_path, outputF1, samtools_reference, inputF, inputFindex, inputList, outputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    inputF = command_meta[step-2][0]["output_file"]
    inputFindex = command_meta[step-1][2]["output_file"]
    outputF1 = "%s/%s-mpileup2.stdout" % (output_dir, sample_name)
    outputF = "%s/%s-mpileup2.bcf" % (output_dir, sample_name)
    samtools_reference = options.samtools_reference
    input_files = [inputF]
    output_files = [outputF, outputF1]
    cmd = "python /opt/galaxy/tools/samtools/samtools_wrapper.py -p '%s/samtools mpileup' --stdout %s -p '-f %s' -d \" \" \"%s\" \"bam\" \"bam_input_0\" -d \"\" \"%s\" \"bam_index\" \"bam_input_0\" -p ' -C 0 -d 250 -q 0 -Q 13 -r MT -g -e 20 -h 100 -L 250 -o 40 > %s '" % (samtools_path, outputF1, samtools_reference, inputF, inputFindex, outputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    step = 11
    command_meta[step] = []
    inputF = command_meta[step-2][3]["output_file"]
    ...
    step = 12
    command_meta[step] = []
    inputF = command_meta[step-3][0]["output_file"]
    outputF1 = "%s/%s-mpileup1.vcf" % (output_dir, sample_name)
    outputF2 = "%s/%s-mpileup1-log.txt" % (output_dir, sample_name)
    outputF1Rest = options.output_mpileup1
    input_files = [inputF]
    output_files = [outputF1, outputF2]
    cmd = "%s/bcftools call -O v -c -o %s %s 2> %s; cp %s %s" % (samtools_path, outputF1, inputF, outputF2, outputF1, outputF1Rest)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF1, "output_files":output_files})

    #1
    #step = 12
    #command_meta[step] = []
    inputF = command_meta[step-3][1]["output_file"]
    outputF1 = "%s/%s-mpileup2.vcf" % (output_dir, sample_name)
    outputF2 = "%s/%s-mpileup2-log.txt" % (output_dir, sample_name)
    outputF1Rest = options.output_mpileup2
    input_files = [inputF]
    output_files = [outputF1, outputF2]
    cmd = "%s/bcftools call -O v -c -o %s %s 2> %s; cp %s %s" % (samtools_path, outputF1, inputF, outputF2, outputF1, outputF1Rest)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF1, "output_files":output_files})
