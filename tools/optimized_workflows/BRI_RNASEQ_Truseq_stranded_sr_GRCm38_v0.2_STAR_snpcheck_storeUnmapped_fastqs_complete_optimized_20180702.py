...
def create_wf_meta(options, output_dir, tmp_dir):
    ...
    cmd = "STAR --genomeLoad NoSharedMemory --genomeDir %s --readFilesIn %s --sjdbGTFfile %s --runThreadN %s --outSAMtype BAM SortedByCoordinate --outSAMunmapped Within --outSAMstrandField intronMotif; cp %s %s; samtools index %s" % (star_reference_db, input1, options.gtf_file, ncpu_step, outputF, outputFrest, outputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files": output_files})
    ...
    #command_meta[step] = []
    inputF = command_meta[step-1][0]["output_files"][0]
    outputF = "%s/%s-star-out.filter.bam" % (output_dir, sample_name)
    input_files = [inputF]
    output_files = [outputF]
    cmd = "python /opt/galaxy/tools/samtools/samtools_filter_bam.py -p '-b' -p '-q 0' -p '-f 4' --input %s --output %s --sorted-bam" % (inputF, outputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    #outputF = "%s/%s-star-out.rd.picard.CollectRnaSeqMetrics_log" % (output_dir, sample_name)
    ...
    #command_meta[step] = []
    inputF = command_meta[step-1][1]["output_file"]
    #outputF = "%s/%s-star-out.rd.picard.alignmentSummaryMetrics" % (output_dir, sample_name)
    ...
    #step = 6
    #command_meta[step] = []
    inputF = command_meta[step-1][3]["output_files"][0]
    ...
    cmd = "bcftools call -O v -c -o %s %s 2> %s; cp %s %s" % (outputF, inputF, outputFstdout, outputF, outputFrest)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})

    #
    #step = 6
    #command_meta[step] = []
    inputF = command_meta[step-1][4]["output_files"][0]
    outputF = "%s/%s.filter.fastq" % (output_dir, sample_name)
    outputFrest = options.output_filtered_fastq
    input_files = [inputF]
    output_files = [outputF, outputFrest]
    cmd = "python /opt/galaxy/tools/picard/picard_SamToFastq_wrapper.py -p ' java -XX:DefaultMaxRAMFraction=1 -XX:+UseParallelGC -jar /mnt/galaxyTools/tools/picard/1.134/picard.jar SamToFastq INPUT=%s VALIDATION_STRINGENCY=LENIENT RE_REVERSE=true INCLUDE_NON_PF_READS=false INCLUDE_NON_PRIMARY_ALIGNMENTS=false FASTQ=%s '; cp %s %s" % (inputF, outputF, outputF, outputFrest)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    #command_meta[step] = []
    ncpu_step = int(ncpu)
    input1 = command_meta[step-1][5]["output_files"][0]
    input_files = [input1]
    ...
    cmd = "python /opt/galaxy/tools/trinity/trinityToolWrapper.py Trinity --output %s/trinity --max_memory %sG --CPU %s --single %s --seqType fq > %s; cp %s %s" % (output_dir, int(jvm_heap/2000), ncpu_step, input1, outputLog, outputF, outputFrest )
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files": output_files})
...
def __main__():
    ...
    parser.add_option( '--output-filtered-fastq', dest="output_filtered_fastq", help="File to save" )
    parser.add_option( '--output-trinity-transcripts', dest="output_trinity_transcripts", help="File to save" )
