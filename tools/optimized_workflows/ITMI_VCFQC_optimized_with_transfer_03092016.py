...
def create_wf_meta(options, output_dir, tmp_dir, datasets):
    ...
    input_bam = datasets[0][2]
    input_vcf = datasets[1][2]
    input_tabix = datasets[2][2]
...
#    tmp_dir = tempfile.mkdtemp(dir=output_dir, prefix="optimized-tmp-")
    ...
    datasets = [[options.source_path_bam, options.destination_path_bam, "%s/%s" % (worker_input_dir, os.path.basename(options.source_path_bam))],
                [options.source_path_vcf, options.destination_path_vcf, "%s/%s" % (worker_input_dir, os.path.basename(options.source_path_vcf))],
                [options.source_path_tbi, options.destination_path_tbi, "%s/%s" % (worker_input_dir, os.path.basename(options.source_path_tbi))]]
    workflows.transfer_multiple_data(options, datasets, output_dir, worker_input_dir)

    # create the meta dictionary of the commands to run
    wf_meta = create_wf_meta(options, output_dir, tmp_dir, datasets)
