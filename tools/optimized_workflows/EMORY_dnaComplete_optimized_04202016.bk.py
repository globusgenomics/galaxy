...
def create_wf_meta(options, output_dir, tmp_dir, input_dir):
    ...
    # transfer BAM to final destination
    #step = 12
    #command_meta[step] = []
    inputF = options.output_bam
    outputF = options.remote_bam
    input_files = [inputF]
    output_files = [outputF]
    #cmd = "python /opt/galaxy/tools/globus/transfer_out_multiple.py %s -o transfer.log --goauth-token \"%s\" --source-ep=\"%s\" --destination-ep=\"%s\" --dataset %s %s %s --deadline=\"240\" --delete-output; rm -rf %s; echo \"Transferred BAM\" > %s" % (options.username, options.goauth_token, options.destination_ep, options.final_ep, inputF, outputF, options.output_transfer_log, inputF, inputF)
    cmd = "python /opt/galaxy/tools/globus/transfer_out_multiple.py %s -o transfer.log --goauth-token \"%s\" --source-ep=\"%s\" --destination-ep=\"%s\" --dataset %s %s %s --deadline=\"240\" --delete-output; echo \"Transferred BAM\" > %s" % (options.username, options.goauth_token, options.destination_ep, options.final_ep, inputF, outputF, options.output_transfer_log, inputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
    ...
    step = 14
    command_meta[step] = []
    inputF = options.output_vcf
    outputF = options.remote_vcf
    input_files = [inputF]
    output_files = [outputF]
    #cmd = "python /opt/galaxy/tools/globus/transfer_out_multiple.py %s -o transfer.log --goauth-token \"%s\" --source-ep=\"%s\" --destination-ep=\"%s\" --dataset %s %s %s --deadline=\"240\"; rm -rf %s; echo \"Transferred VCF\" > %s" % (options.username, options.goauth_token, options.destination_ep, options.final_ep, inputF, outputF, options.output_transfer_log, inputF, inputF)
    cmd = "python /opt/galaxy/tools/globus/transfer_out_multiple.py %s -o transfer.log --goauth-token \"%s\" --source-ep=\"%s\" --destination-ep=\"%s\" --dataset %s %s %s --deadline=\"240\" --delete-output; echo \"Transferred VCF\" > %s" % (options.username, options.goauth_token, options.destination_ep, options.final_ep, inputF, outputF, options.output_transfer_log, inputF)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files":output_files})
...
def __main__():
    ...
    parser.add_option("--final-ep", dest="final_ep", help="Endpoint to transfer final output files to")
    ...
    parser.add_option( '', '--output-bam', dest="output_bam", help='The file to save the output (BAM format)' )
    parser.add_option( '', '--remote-bam', dest="remote_bam", help='Path on destination endpoint (BAM format)' )
    parser.add_option( '', '--output-vcf', dest="output_vcf", help='The file to save the output (interval format)' )
    parser.add_option( '', '--remote-vcf', dest="remote_vcf", help='Path on destination endpoint (VCF format)' )
...
#    # make temp directory 
    ...
    workflows._cleanup_dir(worker_input_dir)
