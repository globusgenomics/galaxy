...
def create_wf_meta(options, output_dir, tmp_dir):
    ...
    output_files = [outputF, outputLog]
    cmd = "python /opt/galaxy/tools/trinity/trinityToolWrapper.py Trinity --output %s/trinity --max_memory %sG --CPU %s --single %s --seqType fq --min_kmer_cov 3 --max_reads_per_graph 20000000 --bflyHeapSpaceMax %sG > %s; touch %s; cp %s %s" % (output_dir, int(jvm_heap/2000), ncpu_step, inputF, int(jvm_heap/2000), outputLog, outputF, outputF, outputFrest )
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files": output_files})
    ...
    ncpu_step = int(ncpu)
    command_meta[step] = []
    input1 = command_meta[step-2][1]["output_files"][0]
    input_files = [input1]
    outputF = "%s/Aligned.sortedByCoord.out.bam" % (output_dir)
    outputFrest = options.output_star_bam
    output_files = [outputF]
    cmd = "STAR --genomeLoad NoSharedMemory --genomeDir %s --readFilesIn %s --sjdbGTFfile %s --runThreadN %s --outSAMtype BAM SortedByCoordinate --outSAMunmapped None --outSAMstrandField intronMotif; cp %s %s" % (star_reference_db, input1, options.gtf_file, ncpu_step, outputF, outputFrest)
    command_meta[step].append({"cl":cmd, "input_files":input_files, "output_file":outputF, "output_files": output_files})
