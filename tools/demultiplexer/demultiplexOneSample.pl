...
my ($adapter_seq, $barcode_file, $no_skip_adp, $no_compress, $compression_level, 
    ...
    $sample_tag, $output_sequence_files);

my @sequence_files = ( );
my @output_sequence_files = ();
...
my $rc = GetOptions(
                    ...
                    "N|allow-Ns"                   => \$allow_Ns,
                    "s|sample-tag=s"               => \$sample_tag,
                    "o|output-sequence-file=s"     => \@output_sequence_files,
                    "h|help"                       => \$help,
                   );

pod2usage(1) if $help;
unless ($rc) {
    print STDERR "Problem with usage,\n\n";
    pod2usage(1);
}
unless (defined($barcode_file)) {
    print STDERR "Please specify -t/--tag-file\n\n";
    pod2usage(2);
}

unless (defined($file_format)) {
    print STDERR "Please specify -f/--file-format\n\n";
    pod2usage(2);
}

unless (defined($sample_tag)) {
    print STDERR "Please specify -s/--sample-tag\n\n";
    pod2usage(2);
}

unless (($tag_end  == 3) || ($tag_end  == 5)) {
    print STDERR "-e/--tag-end must be 3 or 5\n\n";
    pod2usage(2);
}

unless (($tag_read eq '1') || 
                     ($tag_read eq '2') || 
                     ($tag_read eq 'both')) {

    print STDERR "-r/--tag-read must be 1, 2, or both\n\n";
    pod2usage(2)
}


unless ((@sequence_files >= 1) && (@sequence_files <= 2)) {

    print STDERR "Please specify at least one but no more than 2 i/--sequence-file\n\n";
    pod2usage(2);
}

unless (@sequence_files == @output_sequence_files) {

    print STDERR "The number of -i/--sequence-file options must equal the number of -o/--output-sequence-file options\n\n";
    pod2usage(2);
} 
...
if (defined($compression_level)) {
    ...
    if ($no_compress) {
        ...
        unless (($compression_level >= 1) &&
                ($compression_level <= 9)) {
            print STDERR "Problem with compression level\n\n";
            pod2usage(2);
        }
...
# set up sequence counter and check that the sample tag
my $sample_tag_found = 0;
foreach my $tag (keys %Bcodes){

    $BC_counter{$tag} = 0;
    if ($sample_tag_found == $BC_counter{$tag}) {
        $sample_tag_found = 1;
    }
} 

unless ($sample_tag_found) {

   die "ERROR: sample tag '$sample_tag' was not in the tag file '$barcode_file'.  The sample tag must be in the tag file.\n\n"; 

}


# set up output files for sample tag
my $fh_i = open_file_write($output_sequence_files[0]);

$OUT_file_i{$sample_tag}   = $output_sequence_files[0];
$OUT_handle_i{$sample_tag} = $fh_i;

if (defined($sequence_file_j)) {

    my $fh_j = open_file_write($output_sequence_files[1]);
 
    $OUT_file_j{$sample_tag}   = $output_sequence_files[1];
    $OUT_handle_j{$sample_tag} = $fh_j; 

}

# do not need to set up output/files for sequences without a barcode since we will not need these output files
...
while(1) {
	...
	if ((($tag_read == 1) || ($tag_read eq 'both')) &&
           (exists $Bcodes{$match_bc_i})) {
            ...
            # Only write the read to file if it is the sample tag of interest
            if ($match_bc_i eq $sample_tag) {
    ...
    if (defined($sequence_file_j)) {
        ...
        if ((($tag_read == 2) || ($tag_read eq 'both')) &&
           (exists $Bcodes{$match_bc_j})) {
...
sub get_barcodes {
    ...
    if (%Bcodes){ # Print summary of barcodes
        ...
        print "======================", "\n";
        print "demultiplexOneSample.pl $version", "\n";
        print "======================", "\n";
...
demultiplexOneSample.pl - Demultiplex barcoded/indexed scarf/fastq sequence files.  Only reads that uniquely map to the sample tag will be printed to fastq filei(s).  Script can handle fuzzy matching as well as tags of different lengths.

=head1 SYNOPSIS

demultiplexOneSample.pl --tag-file barcodes.txt --file-format scarf --tag-end 5 --tag-read both --sequence-file s_1_sequence.txt

demultiplexOneSample.pl -t barcodes.txt -f scarf --e 5 -r both -i s_1_sequence.txt

demultiplexOneSample.pl --tag-file indexes.txt --file-format fastq --tag-end 3 --tag-read 1 --sequence-file s_1_1_sequence.txt --sequence-file s_1_2_sequence.txt

demultiplexOneSample.pl -t indexes.txt -f fastq -e 3 -r 1 -i s_1_1_sequence.txt -i s_1_2_sequence.txt
...
Specifies the tag sequence of interest.  This tag must also be in the -t/--tag-file.  Only reads that uniquely map to this tag sequence will be printed in the output files.  Required.

=item B<-o/--output-sequence-file>

Specifies the output sequence file(s).  The number of output sequence files must equal the number of input sequence files, i.e., specify 1 if single-read and 2 if paired-end.  Required.
...
B<demultiplexOneSample.pl> will read the input file(s) and bin sequences/reads by the sequence tag (barcode/index) present.  Only reads that uniquely map the sample tag (-s/--sample-tag) will be printed to the output file(s).  
