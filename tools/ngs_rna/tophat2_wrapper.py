...
def __main__():
    ...
    parser.add_option( '', '--bowtie1', action="store_true", dest='bowtie_1' )
    ...
    parser.add_option( '', '--read-edit-dist', dest='read_edit_dist', help='read edit distance' )
    parser.add_option( '', '--read-realign-edit-dist', dest='read_realign_edit_dist', help='read_realign edit distance' )
    ...
    if options.own_file:
        ...
        try:
            os.symlink( options.own_file, index_path + '.fa' )
        except:
    ...
    # figure out if the input files are gzipped fastq or not
    inputFile_format = None
    input1 = None
    input2 = None
    tmp_input_dir = tempfile.mkdtemp()
    if file_type(options.input1) == "gz":
        filename = os.path.basename(options.input1)
        input1 = "%s/%s.gz" % (tmp_input_dir, filename)
        os.symlink(options.input1, input1)
        if options.input2:
            filename = os.path.basename(options.input2)
            input2 = "%s/%s.gz" % (tmp_input_dir, filename)
            os.symlink(options.input2, input2)
    else:
    ...
    if options.settings == 'preSet':
        ...
        try:
            ...
            if options.read_edit_dist:
                opts += ' --read-edit-dist %i' % int( options.read_edit_dist )
            if options.read_realign_edit_dist:
                opts += ' --read-realign-edit-dist %i' % int( options.read_realign_edit_dist )
