    ...
    if len(key) > 0:
        ...
        for hKey, history_dict in completed_meta.iteritems():
            ...
            for dataset_id, dataset_dict in history_dict['datasets'].iteritems():
                if "ENCODE" in dataset_dict['job_name']:
                    continue
            ...
            for job_name in job_names:
                ...
                for raw_dataset in completed_meta[hKey]['raw'][job_name]:
                    ...
                    if os.path.isdir(raw_dataset['path']):
                        ...
                        if os.path.exists(raw_dataset['path']):
                            shutil.move(raw_dataset['path'], "%s/%s/%s" % (options.output_dir, completed_meta[hKey]['sampleName'], source_dir_name))
                            #shutil.copytree(raw_dataset['path'], "%s/%s/%s" % (options.output_dir, completed_meta[hKey]['sampleName'], source_dir_name))
                        else:
                            print "MISSING: %s to %s/%s/%s" % (raw_dataset['path'], options.output_dir, completed_meta[hKey]['sampleName'], source_dir_name)
                    else:
                        if os.path.exists(raw_dataset['path']):
                            shutil.move(raw_dataset['path'], raw_dataset['destination'])
                            #shutil.copy(raw_dataset['path'], raw_dataset['destination'])
                        else:
                            print "MISSING: %s to %s" % (raw_dataset['path'], raw_dataset['destination'])
