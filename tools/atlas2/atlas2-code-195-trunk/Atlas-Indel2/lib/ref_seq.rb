...
class Ref_seq
	...
	def get_base(chr, coor)
		if( @fai != nil )
			...
			base = @file.read(1)
			if(base.nil?)
				STDERR.puts "WARNING: Illegal reference coordinate: #{chr}:#{coor}, returning N"
				return "N"
			end
			return base
		else
		...
		if( @fai != nil )
			...
			while output.length <= stop-start
				...
				if( nextline =~ /^>(\S+)/ || nextline == :eof )
					STDERR.puts "WARNING: Illegal reference coordinate: #{chr}:#{start+output.length}, returning N"
					while output.length <= stop-start
						output << "N"
					end
				else
