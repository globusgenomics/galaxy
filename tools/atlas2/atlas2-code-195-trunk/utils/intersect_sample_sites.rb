...
while(line1 != :EOF && line2 != :EOF)
	...
	if(vcf_line1.chr == vcf_line2.chr && vcf_line1.pos == vcf_line2.pos)
		if(allele_matches(vcf_line1, vcf_line2))
			samples = vcf_line2.info[:"samples"]
			samples = [samples] if samples.class != Array
			found_sample = false
			samples.each do |sample_name|
				if(vcf_line1.samples[sample_name.to_s][:"GT"] =~ /[1-9]/)
					found_sample = true
					break
				end
			end
			writer1.puts line1 if found_sample
			writer2.puts line2 if found_sample
		end
		line1 = get_line(file1)
		line2 = get_line(file2)
		next
