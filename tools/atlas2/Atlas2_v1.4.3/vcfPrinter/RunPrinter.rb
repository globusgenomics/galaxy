...
raise RuntimeError, "settings.rb does not exist!" if !File.exist? ("#{File.dirname(__FILE__)}/settings.rb")
...
def quickPrinter(vcfs, out_file, onlyPass=false)
  ...
  collapseVariants(out_file)
  STDERR.puts "Done!"
end

def sortVCF(outFile)
  #Function meant to use only with human genomes
  #Sort vcf using unix sort
  #Using combination if sed,grep and sort to sort the vcf file
  tfile = Tempfile.new('vcfPrinter')
  #substitute X,Y,M with 23,24,25 using sed (numerical sort to work)
