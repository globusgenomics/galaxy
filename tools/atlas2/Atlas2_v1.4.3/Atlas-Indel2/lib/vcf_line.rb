...
class Vcf_line
	include Comparable
	attr_accessor  :chr, :pos, :id, :ref, :alt, :qual, :filter, :info, :format, :samples, :source_file, :sample_names
	...
	def self.read_line(line_str, filter, labels, source_file=nil)
		begin
			...
			raise "line #{vcf_line.chr}:#{vcf_line.pos} size #{line.length} does not match labels size #{labels.length}" if line.length != labels.length
	...
	def print
		return "#{@chr}\t#{@pos}\t#{@id}\t#{@ref}\t#{@alt.join(',')}\t#{@qual}\t#{@filter}\t#{print_info}\t#{@format.join(':')}\t#{print_samples}".chomp
	end
	...
	def <=> (partner)
		if(get_chrom_num > partner.get_chrom_num)
			return 1
		elsif(get_chrom_num < partner.get_chrom_num)
			return -1
		end
		if(@pos > partner.pos)
			return 1
		elsif(@pos < partner.pos)
			return -1
		end
		return 0;
	end #<=>
	...
	def parse_sample(sample_str, sample_name)
		...
		if(sample_arr.size != @format.size)
			raise "The sample #{sample_name} format #{sample_arr.size} does not match the specified format #{@format.size} -- #{sample_arr}"
		end
