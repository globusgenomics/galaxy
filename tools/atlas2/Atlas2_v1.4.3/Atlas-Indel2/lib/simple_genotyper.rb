...
class Simple_genotyper
	...
	def initialize( cutoffs=[0.03,0.6,0.9,0.5,3] )
	...
	def genotype(ref_depth, alt_depth, total_depth, allele=1)
		...
		if(alt_depth/n >= @homo_var_cutoff)
			return "#{allele}/#{allele}"
		elsif(alt_depth/n >= @heter_cutoff)
			return './.' if allele == '.'
			return "#{allele}/0"
		elsif(ref_depth/n >= @homo_var_cutoff)
			return '0/0'
		elsif(ref_depth/n >= @heter_cutoff)
			return '0/.'
		else
			return './.'
		end
...
if __FILE__ == $0
	...
	require 'vcf_line'
	if(ARGV.length < 1)
		puts "USAGE: ruby simple_genotyper.rb [vcf_file] homo_alt_cutoff [0.6]"
		exit(0)
	end
	...
	genotyper = Simple_genotyper.new([0.03,cutoff,0.9,0.5,3])
	labels=nil
