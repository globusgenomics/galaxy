...
class Atlas_indel2
	...
	def indicate_progress(chr, coor)
		puts "Chromosome:#{chr}  Coor:#{coor}                           \r"
		STDOUT.flush
		STDERR.flush
		@outfile.flush
	end
	...
	def print_nonvar_site(include_site, depths, ref_depths)
		...
		geno = @genotyper.genotype(ref_depth, 0, depth)
                puts "#{include_site[0]}\t#{include_site[1]}"
		ref_seq = @reference.get_base(include_site[0], include_site[1])
		if(depth>0)
			@outfile.puts "chr#{include_site[0]}\t#{include_site[1]}\t.\t#{ref_seq}\t.\t.\tNonVar\tP=.;ReqIncl\tGT:VR:RR:DP:GQ\t#{geno}:0:#{ref_depth}:#{depth}:."
		else
			@outfile.puts "chr#{include_site[0]}\t#{include_site[1]}\t.\t#{ref_seq}\t.\t.\tNoData\tP=.;ReqIncl\tGT:VR:RR:DP:GQ\t#{geno}:0:#{ref_depth}:#{depth}:."
		end
	end
